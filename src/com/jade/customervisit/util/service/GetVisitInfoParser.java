...
public class GetVisitInfoParser extends AbsBaseParser<GetVisitInfoResult> {

    public GetVisitInfoParser(IAsyncListener<GetVisitInfoResult> listener) {
        super(listener);
    }
            ...
            if (json != null) {
                ...
                if (!json.isNull(ResponseKey.DATA_INFO)) {
                    ...
                    for (int i = 0; i < size; i++) {
                        ...
                        String username = obj
                                .optString(ResponseKey.CUSTOMER_NAME);
                        String arriveTime = obj
                                .optString(ResponseKey.ARRIVE_TIME);
                        String leaveTime = obj
                                .optString(ResponseKey.LEAVE_TIME);
                        ...
                        if (!obj.isNull(ResponseKey.IMAGE_LIST)) {
                            JSONArray arr = obj
                                    .optJSONArray(ResponseKey.IMAGE_LIST);
                            int ssize = arr.length();
                            for (int j = 0; j < ssize; j++) {
                                JSONObject imgObj = arr.getJSONObject(j);
                                String imageUrl = imgObj
                                        .optString(ResponseKey.IMAGE_URL);
