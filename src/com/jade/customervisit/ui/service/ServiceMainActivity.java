...
public class ServiceMainActivity extends BaseActivity implements OnClickListener {
	...
	private void queryContent() {
        ...
        // 开始请求列表数据
        httpHandler = ServiceManager.queryContent(serviceContent.getServiceId(), callback);
	}
	...
	private void queryServiceStatus() {
		RequestListener<QueryServiceStatusResult> callback = new RequestListener<QueryServiceStatusResult>() {
            ...
            @Override
            public void onSuccess(int statusCode, QueryServiceStatusResult result) {
                if (result != null) {
                    if (result.isSuccesses()) {
        ...
        // 开始请求服务状态信息
        httpHandler = ServiceManager.queryServiceStatus(serviceContent.getServiceId(), callback);
	}
	...
	public void onClick(View v) {
        switch (v.getId())
        {
            ...
            case R.id.service_contents:
                Intent contentsIntent = new Intent(this, ServiceContentActivity.class);
            ...
            case R.id.service_evaluation:
                Intent evaluationIntent = new Intent(this, MipcaActivityCapture.class);
                evaluationIntent.putExtra("type", Constants.PRAISE);
                evaluationIntent.putExtra("serviceId", serviceContent.getServiceId());
